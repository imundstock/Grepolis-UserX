...
class AntiRage extends ModernUtil {
	GOODS_ICONS = {
		...
		ares: 'js-power-icon.animated_power_icon.animated_power_icon_45x45.power_icon45x45.power.bloodlust',
        hera: 'js-power-icon.animated_power_icon.animated_power_icon_45x45.power_icon45x45.power.desire',
        hades: 'js-power-icon.animated_power_icon.animated_power_icon_45x45.power_icon45x45.power.resurrection',
	};
		...
		uw.$.Observer(uw.GameEvents.window.open).subscribe((e, data) => {
			...
			const addSpell = async () => {
				...
				if (!spellMenu) {
					if (max > 0) {
						max -= 1;
						setTimeout(addSpell, 500);
					}
					return;
				}
                this.arrival_time = $('.arrival_time')[0].innerHTML.slice(25,33);
	...
	handleGod = good => {
		...
		godEl.click(() => {
			...
			this.loop_funct = setInterval(this.clicker, 0, powerEl);
	...
	trigger = () => {
		setTimeout(() => {
            ...
            $('.js-god-box[data-god_id="ares"]').find('.powers').append(`
            <div id="enchanted_ares_army" class="js-power-icon animated_power_icon animated_power_icon_45x45 power_icon45x45 power ares_army" style="filter: brightness(70%) sepia(104%) hue-rotate(14deg) saturate(1642%) contrast(0.8)">
                <div class="extend_spell">
                    <div class="gold"></div>
                </div>
                <div class="js-caption"></div>
            ...
            $('.js-god-box[data-god_id="poseidon"]').find('.powers').append(`
            <div id="enchanted_storm" class="js-power-icon animated_power_icon animated_power_icon_45x45 power_icon45x45 power sea_storm" style="filter: brightness(70%) sepia(104%) hue-rotate(14deg) saturate(1642%) contrast(0.8)">
                <div class="extend_spell">
                    <div class="gold"></div>
                </div>
                <div class="js-caption"></div>
            ...
            <table class="popup" id="popup_div" cellpadding="0" cellspacing="0" style="display: block; left: 243px; top: 461px; opacity: 1; position: absolute; z-index: 6001; width: auto; max-width: 400px;">
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="popup_middle_middle" id="popup_content" style="width: auto;">
                            <div class="temple_power_popup ">
                                <div class="temple_power_popup_image power_icon86x86 sea_storm" style="filter: brightness(70%) sepia(104%) hue-rotate(14deg) saturate(1642%) contrast(0.8)"></div>
                                <div class="temple_power_popup_info">
                                    <h4>Enchanted Sea Storm</h4>
                                    <p> An Enchanted version of the normal sea storm </p>
                                    <p> Made for who try to troll with the autoclick </p>
                                    <p><b> Cast Purification and sea storm at the same time </b></p>
                                    <div class="favor_cost_info">
                                        <div class="resource_icon favor"></div>
                                        <span>280 poseidon + 200 artemis</span>
                                    </div>
                                </div>
                            </div>
                        </td>
            ...
            <table class="popup" id="popup_div" cellpadding="0" cellspacing="0" style="display: block; left: 243px; top: 461px; opacity: 1; position: absolute; z-index: 6001; width: auto; max-width: 400px;">
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="popup_middle_middle" id="popup_content" style="width: auto;">
                            <div class="temple_power_popup ">
                                <div class="temple_power_popup_image power_icon86x86 ares_army" style="filter: brightness(70%) sepia(104%) hue-rotate(14deg) saturate(1642%) contrast(0.8)"></div>
                                <div class="temple_power_popup_info">
                                    <h4>Enchanted Ares Army</h4>
                                    <p> An Enchanted version of the ares army </p>
                                    <p> Made for who try to troll with the autoclick </p>
                                    <p><b> Cast Purification and ares army at the last second</b></p>
                                    <div class="favor_cost_info">
                                        <div class="resource_icon favor"></div>
                                        <span>380 ares + 200 artemis</span>
                                    </div>
                                </div>
                            </div>
                        </td>
            ...
            this.year = parseInt(new Date(Date.now()).getFullYear());
            this.month = parseInt(new Date(Date.now()).getMonth());
            this.day = parseInt(new Date(Date.now()).getDate());

            this.hour = parseInt(this.arrival_time.split(":")[0]);
            this.minutes = parseInt(this.arrival_time.split(":")[1]);
            this.seconds = parseInt(this.arrival_time.split(":")[2]);


            this.arrival_time = Date.parse(new Date(this.year,this.month,this.day,this.hour,this.minutes,this.seconds));

			const { artemis_favor, zeus_favor , poseidon_favor, ares_favor } = uw.ITowns.player_gods.attributes;
			const enable = artemis_favor >= 200 && zeus_favor >= 300;
            const enable_poseidon = artemis_favor >= 200 && poseidon_favor >= 280;
            const enable_ares = artemis_favor >= 200 && ares_favor >= 380;
			if (!enable) $('#enchanted_rage').css('filter', 'grayscale(1)');
            if (!enable_poseidon) $('#enchanted_storm').css('filter', 'grayscale(1)');
             if (!enable_ares) $('#enchanted_ares_army').css('filter', 'grayscale(1)');
            ...
            // TODO: disable if not enable
			$('#enchanted_ares_army').on({
				click: () => {
					if (!enable_ares) return;
					this.enchanted('ares');
				},
				mouseenter: event => {
					$('#popup_div_curtain').html(html_ares_army);
					const $popupDiv = $('#popup_div');
					const offset = $popupDiv.offset();
					const height = $popupDiv.outerHeight();
					const width = $popupDiv.outerWidth();
            ...
            $('#enchanted_storm').on({
                click: () => {
                    if (!enable_poseidon) return;
                    this.enchanted('poseidon');
                },
                mouseenter: event => {
                    $('#popup_div_curtain').html(html_sea_storm);
                    const $popupDiv = $('#popup_div');
                    const offset = $popupDiv.offset();
                    const height = $popupDiv.outerHeight();
                    const width = $popupDiv.outerWidth();
	...
	clicker = el => {
        ...
        el.click();
		//let delta_time = 1;
		//let rand = 0 + Math.floor(Math.random() * delta_time);
		clearInterval(this.loop_funct);

		this.loop_funct = setInterval(this.clicker, 0, el);
	};
	...
	enchanted = async type => {
        ...
        if (type === 'poseidon') {
            this.cast(this.command_id, 'cleanse');
            await this.sleep(1);
            this.cast(this.command_id, 'sea_storm');
        }
        if (type === 'ares') {
            console.log("ola1");
            setTimeout(async function(id) {
                ...
                setTimeout(async function(id) {
                    ...
                    uw.gpAjax.ajaxPost('frontend_bridge', 'execute', data);},50,id);
            },this.arrival_time-Date.now()-500,this.command_id);
            console.log("ola5");
...
/*class AntiRage extends ModernUtil {
	...
	trigger = () => {
		setTimeout(() => {
			...
			const { artemis_favor, zeus_favor , poseidon_favor } = uw.ITowns.player_gods.attributes;
			const enable = artemis_favor >= 200 && zeus_favor >= 300;
            const enable_poseidon = artemis_favor >= 200 && poseidon_favor >= 280;
			if (!enable) $('#enchanted_rage').css('filter', 'grayscale(1)');
            if (!enable_poseidon) $('#enchanted_storm').css('filter', 'grayscale(1)');
...
class AutoBuild extends ModernUtil {
	...
	settings = () => {
        ...
        let html_text = `<div class="game_border" style="margin-bottom: 20px">
            <div class="game_border_top"></div>
            <div class="game_border_bottom"></div>
            <div class="game_border_left"></div>
            <div class="game_border_right"></div>
            ...
            <label for="groups">save for group:</label>
            <select name="group" id="group__select">`;

        for (let group in uw.ITowns.getTownGroups()){
            html_text += `<option value="${group}">${uw.ITowns.getTownGroups()[group].name}</option>`
        }

        html_text += `</select>
        <button type="button" onclick="window.modernBot.autoBuild.save_group()">Save</button>
        <label for="buildings">1st building:</label>
        <select name="1st_building" id="1st_building">
        <option value="trade_office">trade_office</option>
        <option value="oracle">oracle</option>
        <option value="statue">statue</option>
        <option value="tower">tower</option>
        </select>
        <label for="2nd_buildings">2nd building:</label>
        <select name="2nd_building" id="2nd_building">
    ...
    save_group = () => {
        let town_id_aux = uw.ITowns.getCurrentTown().id;
        let town_aux = uw.ITowns.towns[town_id_aux];

		const town_buildings = this.towns_buildings?.[town_id_aux] ?? { ...town_aux.buildings()?.attributes } ?? {};
        this.saved_builds[$('#group__select').val()] = town_buildings;
	...
	setPolisInSettings = town_id => {
        ...
        for (let group in uw.ITowns.getTownGroups()){
                for(let group_town_id in uw.ITowns.getTownGroups()[group].towns){
                    if(town.id == group_town_id && uw.ITowns.getTownGroups()[group].name != "Todos" && uw.ITowns.getTownGroups()[group].name!= "Sem grupos" && this.saved_builds[group] != null ){
                        town_buildings = this.saved_builds[group];
	...
	toggle = () => {
		...
		if (!(town.id.toString() in this.towns_buildings)) {
			...
			let buildins = ['main', 'storage', 'farm', 'academy', 'temple', 'barracks', 'docks', 'market', 'hide', 'lumber', 'stoner', 'ironer', 'wall'];//,$('#1st_building').val(),$('#2nd_building').val()];


            buildins.forEach(e => {//ver aqui
...
class AutoParty extends ModernUtil {
	constructor(c, s) {
		...
		this.active_types = this.storage.load('ap_types', { festival: false, procession: false, theater: false, games: false });
	...
	settings = () => {
        ...
        <div class="game_border" style="margin-bottom: 20px">
            ...
            <div id="autoparty_types" class="split_content">
                ...
                ${this.getButtonHtml('autoparty_theater', 'Theater', this.triggerType, 'theater')}
                ${this.getButtonHtml('autoparty_games', 'Games', this.triggerType, 'games')}
                </div>
    ...
    checkTheater = async () => {
		...
		if (this.single) {
			for (let town_id in uw.ITowns.towns) {
				...
				let town = uw.ITowns.towns[town_id];
				if (town.getBuildings().attributes.academy < 30 || town.getBuildings().attributes.theater == 0) continue;
				let { wood, stone, iron } = town.resources();
    ...
    checkGames = async () => {
		let max = 10;
		let games = this.getCelebrationsList('games');
		if (this.single) {
			for (let town_id in uw.ITowns.towns) {
				if (games.includes(parseInt(town_id))) continue;
				let town = uw.ITowns.towns[town_id];
				if (town.getBuildings().attributes.academy < 30 || $('.gold_amount')[0].innerHTML < 50) continue;
				this.makeCelebration('games', town_id);
				await this.sleep(750);
				max -= 1;
				/* Prevent that the promise it's to long */
				if (max <= 0) return;
	...
	main = async () => {
		...
		if (this.active_types['games']) await this.checkGames();
	};
...
class AutoTrain extends ModernUtil {
	...
	settings = () => {
        ...
        <div class="game_border" style="margin-bottom: 20px">
            ...
            <div class="split_content">
                ...
                ${this.getButtonHtml('train_percentuals_1', '60%', this.handlePercentual, 1)}
                ${this.getButtonHtml('train_percentuals_2', '70%', this.handlePercentual, 2)}
                ${this.getButtonHtml('train_percentuals_3', '80%', this.handlePercentual, 3)}
                </div>
            </div>
        </div>
	...
	getTroopCount = (troop, town_id) => {
        ...
        let favor = uw.GameData.units[troop].favor;
		...
		let current = parseInt(Math.min(w, s, i, f));
		...
		let max = parseInt(Math.min(w_max, s_max, i_max,favor_max) * 0.9); // 0.8 it's the full percentual -> 80%
...
class AutoPuri extends ModernUtil {
    constructor(c, s) {
        super(c, s);

        this.activePolis = this.storage.load('autoPuri_active', 0);
